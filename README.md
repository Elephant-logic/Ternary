...
The root `render.yaml` now uses Render's **free** web-service plan and does **not** attach a paid persistent disk.
...
Render Free files are ephemeral, so local JSON alone cannot reliably survive a Render redeploy/restart. For durable free-tier recovery, the Blueprint asks for:

- `UPSTASH_REDIS_REST_URL`
- `UPSTASH_REDIS_REST_TOKEN`

Ternary stores **one versioned JSON runtime snapshot** (state + hash-chained events) through Upstash's REST API and restores it **before** `AppState` and the worker boot. `/status` exposes whether remote JSON persistence is enabled, whether the instance restored from it, the last-save time, and any persistence error.

If the Upstash variables are absent, Ternary still runs and saves local JSON, but that state is not durable across Render filesystem resets.
...
3. For restart-safe no-SQL state, create a free Upstash Redis database and paste its REST URL/token when Render prompts.
...
5. The service starts in **PAPER** mode.
...
The current suite includes the original safety/integration/property checks plus explicit no-SQL persistence tests proving that:
